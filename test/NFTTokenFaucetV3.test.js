...
const factoryWithLegacy = async (seeder) => {
  const token = await MockERC20.new();
  const nft = await MockERC721.new();
  const lock = await TokenLockManagerV2.new();
  const legacyLock = await TokenLockManager.new(nft.address);
  const legacyFaucet = await NFTTokenFaucetV2.new({
    token: token.address,
    nft: nft.address,
    lock: legacyLock.address,
  });
  const createFaucet = () =>
    NFTTokenFaucetV3.new({
      token: token.address,
      lock: lock.address,
      legacy: {
        seeder,
        faucet: legacyFaucet.address,
        nft: nft.address,
        lock: legacyLock.address,
      },
    });
  await token.approve(legacyFaucet.address, INFINITY);

  return { token, nft, createFaucet, lock, legacyLock, legacyFaucet };
};
...
contract('NFTTokenFaucetV3', (accounts) => {
  ...
  describe('legacy migration', () => {
    it('shoud migrate legacy tokens over', async () => {
      const { nft, token, legacyFaucet, createFaucet } = await factoryWithLegacy(a1);
      await token.mint(toWei('100000'));
      await nft.mint('1');
      await nft.mint('2');

      await setNetworkTime('2021-04-20');
      await legacyFaucet.seed('1', toWei('1000'), 10);
      await setNetworkTime('2021-05-20');
      const faucet = await createFaucet();
      assert.equal(await faucet.allTokensCount(), 1);
      assert.equal(await faucet.tokensBySeederCount(a1), 1);

      assert.equal((await faucet.allTokens(0)).tokenId, '1');
      assert.equal((await faucet.allTokens(0)).nft, nft.address);
      assert.equal((await faucet.tokensBySeeder(a1, 0)).tokenId, '1');
      assert.equal((await faucet.tokensBySeeder(a1, 0)).nft, nft.address);
    });
    it('should revert if attempting to claim legacy token', async () => {
      const { nft, token, legacyFaucet, createFaucet } = await factoryWithLegacy(a1);
      await token.mint(toWei('100000'));
      await nft.mint('1');
      ...
      await legacyFaucet.seed('1', toWei('1000'), 10);
      const faucet = await createFaucet();
      const task = faucet.claim(nft.address, '1', toWei('100'));
      await truffleAssert.fails(task, truffleAssert.ErrorType.REVERT, 'legacy token');
    });
    it('shoud proxy data from legacy', async () => {
      const { nft, token, legacyFaucet, createFaucet, legacyLock } = await factoryWithLegacy(a1);
      await token.mint(toWei('100000'));
      await nft.mint('1');
      await nft.mint('2');
      ...
      assert.equal((await faucet.getToken(nft.address, '1')).balance, toWei('10000')); // balance pass thru
      assert.equal((await faucet.getToken(nft.address, '1')).claimable, toWei('10000')); // claimable pass thru
      assert.equal((await faucet.getToken(nft.address, '1')).unlocksAt, 0); // lock passes thru
      assert.equal(
        (await faucet.getToken(nft.address, '1')).lastClaimAt,
        (await faucet.getToken(nft.address, '1')).seededAt
      ); // last claim psses thru

      await legacyFaucet.claim('1', toWei('1000'));
      assert.equal((await faucet.getToken(nft.address, '1')).balance, toWei('9000')); // balance pass thru
      assert.equal((await faucet.getToken(nft.address, '1')).claimable, toWei('9000')); // claimable pass thru
      await legacyLock.lockToken('1');
      assert.isAbove(Number((await faucet.getToken(nft.address, '1')).unlocksAt), 0); // lock passes thru
      assert.isAbove(
        Number((await faucet.getToken(nft.address, '1')).lastClaimAt),
        Number((await faucet.getToken(nft.address, '1')).seededAt)
      ); // last claim psses thru
    });
  });
});
